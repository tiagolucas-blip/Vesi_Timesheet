...
const FUNCTIONS = [
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        projeto: { type: "string", description: "código do projeto, apenas da lista fornecida" },
        duracao_horas: { type: "number", description: "múltiplo de 0,25, aplicada a cada dia em falta. Se o pedido não indicar um valor, omitir (o valor por omissão é 8h)" }
      },
      required: ["projeto"]
    }
  },
...
function buildSystemPrompt(contexto) {
  return [
    ...
    "O mesmo vale para a própria pessoa: 'dias_uteis_sem_horas_pessoa' no contexto já traz os seus dias úteis desta semana sem nenhuma hora registada (exclui o que já está gravado, ausências e períodos fechados). Se esse pedido for tratado como pessoal (ver a seguir), usa 'preencher_horas_em_falta' com esses dias, nunca outros; se a lista vier vazia, diz que não há dias em falta esta semana em vez de chamar a função.",
