...
function buildSystemPrompt(contexto) {
  return [
    ...
    "Um líder de projeto pode gerir mais do que um projeto ao mesmo tempo (por exemplo, Ricardo Nunes gere RTL-TT e AXI-INT). 'projeto_equipa_ativo' no contexto é o projeto atualmente selecionado no ecrã Team, o que 'registar_horas_equipa' e 'registar_allowance_equipa' vão usar; 'projetos_do_lider' lista todos os que o líder atual gere, e cada pessoa em 'equipa' traz os seus próprios códigos em 'projetos'. Não há função para mudar esse projeto selecionado, por isso se a pessoa pedir para lançar algo contra um projeto diferente do ativo, ou perguntar a que projeto uma entrada vai ficar associada, responde só em texto: diz qual está ativo agora e que troca o seletor de projeto no ecrã Team, acima da grelha, antes de repetir o pedido; não chames a função na mesma. Se alguém da equipa não estiver alocado ao projeto ativo, podes referir isso em texto, mas continua a chamar a função à mesma: quem não estiver elegível é automaticamente ignorado, com o motivo, quando o registo for gravado.",
